...
async function silentVoteMisterLauncher(project) {
    ...
    if (response.doc.querySelector('.container div.alert.alert-danger') != null) {
        if (response.doc.querySelector('.container div.alert.alert-danger').textContent.includes('Вы уже голосовали за этот проект')) {
            ...
            endVote({message: response.doc.querySelector('.container div.alert.alert-danger').textContent}, null, project)
        }
        ...
        if (response.doc.querySelector('.container div.alert.alert-success').textContent.includes('Спасибо за Ваш голос!')) {
            ...
            endVote({message: response.doc.querySelector('.container div.alert.alert-success').textContent}, null, project)
        }
    } else {
